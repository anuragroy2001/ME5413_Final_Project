...
class Image_segmentation:
    ...
    def ocr_detection_tranfrom(self): 
        ...
        for detection in result:
            ...
            # Hardcoded filtering: suppress common OCR misclassifications
            if detection[1] == "3":
                if 8 in self.detected_numbers_positions:
                    for pos in self.detected_numbers_positions[8]:
                        dist = np.linalg.norm([pos[0] - X, pos[1] - Y, pos[2] - Z])
                        if dist < self.misclass_dist_thresh:
                            rospy.loginfo(f"Suppressed '3' misread — nearby '8' detected at {dist:.2f}m")
                            continue  # Skip this detection

            if detection[1] == "1":
                if 7 in self.detected_numbers_positions:
                    for pos in self.detected_numbers_positions[7]:
                        dist = np.linalg.norm([pos[0] - X, pos[1] - Y, pos[2] - Z])
                        if dist < self.misclass_dist_thresh:
                            rospy.loginfo(f"Suppressed '1' misread — nearby '7' detected at {dist:.2f}m")
                            continue  # Skip this detection
